...
export function NoteConnections({ notes, connections, canvasRef, editorRef, highlights, zoom }: NoteConnectionsProps) {
    ...
    useEffect(() => {
        ...
        // Re-run when the editor scrolls (highlight positions shift relative to canvas)
        const editor = editorRef;
        if (editor) editor.addEventListener('scroll', calculateLines, { passive: true });

        return () => {
            if (editor) editor.removeEventListener('scroll', calculateLines);
        };

    }, [notes, connections, canvasRef, editorRef, highlights, zoom]);
